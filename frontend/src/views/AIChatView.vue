    ...
    <section class="chat-area">
      <header class="chat-header">
        ...
        <div class="header-actions">
          ...
          <button v-else class="btn" @click="runSession" :disabled="runDisabled">重新运行</button>
      ...
      <section v-if="activeTab === 'overview'" class="panel overview">
          ...
          <div class="final-result">
            ...
            <div v-if="finalResult">
              ...
              <div v-if="finalResult.code || finalResult.code_str">
                ...
                <div class="artifact-meta lang-line">语言：<span class="lang-chip">{{ codeLangLabel }}</span></div>
                <div v-if="!isEditing">
        ...
        <!-- 计划/决策面板 -->
        <div class="plans-panel" v-if="planScores.length">
          <h3>计划 / 决策</h3>
          <ul class="plan-list">
            <li v-for="p in planScores" :key="p.idx" :class="['plan-item', p.status]">
              <div class="plan-line">
                <span class="plan-idx">计划 {{ p.idx + 1 }}</span>
                <span class="plan-score">分数 {{ p.score?.toFixed(2) ?? '-' }}</span>
                <span class="plan-status" :class="p.status">{{ p.status }}</span>
                <span class="plan-attempts">尝试 {{ p.attempts || 0 }}</span>
              </div>
      ...
      <section v-if="activeTab === 'tasks'" class="panel tasks-panel">
        <h3>子任务列表（按角色分组）</h3>
        <div v-for="group in groupedTasks" :key="group.role" class="task-group">
          <div class="group-title">{{ group.label }} ({{ group.items.length }})</div>
          <ul class="task-list">
            <li v-for="t in group.items" :key="t.id" :class="['task-item', roleColor(t.assigned_role_id)]">
              <div class="task-header" @click="toggleTaskCollapse(t.id)">
                <div class="task-title">
                  <span class="task-toggle" :class="{collapsed: isTaskCollapsed(t.id)}"></span>
                  {{ t.title }}
                </div>
                <div class="task-meta">状态: {{ t.status }} · 重试: {{ t.attempt_count || 0 }}/{{ t.max_attempts || '-' }}</div>
              </div>
              <div class="task-body" v-if="taskResultText(t)">
                <div class="result" v-if="!isTaskCollapsed(t.id)">
                  ...
                  <div v-if="t.result?.tests" class="tests">测试: {{ t.result.tests.pass ? '通过' : '失败' }}</div>
                </div>
      ...
      <section v-if="activeTab === 'logs'" class="panel logs-panel">
        <h3>执行日志</h3>
        <ul class="log-list">
          <li v-for="l in logs" :key="l.id" :class="['log-item', logClass(l)]">
...
const editCode = ref('');
const planScores = ref([]);
// 子任务按角色分组，供模板使用
const groupedTasks = computed(() => {
  const groups = [
    {role: 'retriever', label: 'Retriever', items: []},
    {role: 'planner', label: 'Planner', items: []},
    {role: 'coder', label: 'Coder', items: []},
    {role: 'debugger', label: 'Debugger', items: []},
    {role: 'other', label: '其他', items: []},
  ];
  (visibleTasks.value || []).forEach(t => {
    const role = getRoleName(t.assigned_role_id);
    const g = groups.find(g => g.role === role) || groups.find(g => g.role === 'other');
    g.items.push(t);
  });
...
const codeLangLabel = computed(() => {
  const fr = finalResult.value || {};
  const declared = (fr.language || fr.lang || '').toString();
  if (declared) return declared;
  const text = (fr.code_str || fr.code || '').toString();
  const lower = text.slice(0, 800).toLowerCase();
  if (lower.includes('public class') || lower.includes('static void main')) return 'java';
  if (lower.includes('#include <')) return 'cpp';
  if (lower.includes('def ') || lower.includes('import ') && lower.includes(' as ')) return 'python';
  return 'plaintext';
});
...
function highlightCode(code) {
  try {
    const fr = finalResult.value || {};
    const lang = (fr.language || fr.lang || codeLangLabel.value || 'plaintext').toString().toLowerCase();
    const used = ['python','java','cpp','c++','javascript','typescript','go','cs','php','ruby','kotlin'].includes(lang) ? lang : 'plaintext';
    const res = hljs.highlight(code || '', {language: used, ignoreIllegals: true});
    return res.value;
  } catch (e) {
...
function scrollMessages() {
  // placeholder: page layout no longer uses a chat messages list. Keep to avoid errors.
}
...
function handleSSEEvent(payload) {
  ...
  if (payload.plan_scores || payload.plans) {
    planScores.value = payload.plans || payload.plan_scores || [];
  }
}
...
const logClass = (l) => {
  ...
  if (msg.includes('执行动态计划')) return 'log-plan';
  if (msg.includes('失败')) return 'log-error';
  if (msg.includes('完成')) return 'log-success';
  return '';
};
